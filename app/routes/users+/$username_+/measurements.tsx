...
export async function loader({ params }: DataFunctionArgs) {
	const timings = makeTimings('measurements loader')
	...
	const measurements = await time(
		...
		{ timings, type: 'find measurements' },
	)
	return json(
		{ owner, measurements },
					...
					<div className="absolute inset-0 flex flex-col">
						...
						>
							...
							<h1 className="text-center text-base font-bold md:text-lg lg:text-left lg:text-2xl">
								{ownerDisplayName}'s Measurements
							</h1>
						</Link>
						<ul className="overflow-y-auto overflow-x-hidden pb-12">
							{user?.id === data.owner.id ? (
								<li>
									...
									>
										<Icon name="plus">New Measurement</Icon>
									</NavLink>
								</li>
							) : null}
							{data.measurements.map(measurement => (
								<li key={measurement.id}>
									...
									>
										{format(parseISO(measurement.createdAt), 'MM/dd/yyyy HH:mm')}
									</NavLink>
								</li>
							))}
						</ul>
